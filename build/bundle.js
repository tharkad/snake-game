    ...
    function create_fragment$3(ctx) {
    	...
    	return {
    		c() {
    			...
    			t7 = text("Speed ");
    			t8 = text(/*speed*/ ctx[5]);
    			attr(h1, "class", "svelte-cp7ayt");
    			set_style(main, "width", boardWidth + "px");
    			set_style(main, "height", boardHeight + "px");
    			attr(main, "class", "svelte-cp7ayt");
    			attr(h20, "class", "svelte-cp7ayt");
    			attr(h21, "class", "svelte-cp7ayt");
    		},
    		...
    		p(ctx, [dirty]) {
    			...
    			if (!current || dirty & /*speed*/ 32) set_data(t8, /*speed*/ ctx[5]);
    		},
    		i(local) {
    			if (current) return;
    ...
    function instance$3($$self, $$props, $$invalidate) {
    	...
    	function gameLoop() {
    		...
    		if (isCollide(newHead, { left: foodLeft, top: foodTop })) {
    			...
    			$$invalidate(7, interval = Math.floor(scale / newSpeed));
    			console.log(interval);
    			$$invalidate(5, speed = Math.floor(Math.floor(scale / interval * 10000) / 100));
    		}
    	...
    	function moveFood() {
    		...
    		while (!goodFood) {
    			$$invalidate(1, foodTop = Math.floor(Math.random() * (Math.floor(boardHeight / scale) - 1)) * scale);
    			$$invalidate(0, foodLeft = Math.floor(Math.random() * (Math.floor(boardWidth / scale) - 1)) * scale);
    	...
    	function resetGame() {
    		...
    		$$invalidate(7, interval = Math.floor(scale * 8));
    		$$invalidate(3, snakeBodies = [{ left: scale * 2, top: 0 }, { left: scale, top: 0 }, { left: 0, top: 0 }]);
    	}
    	...
    	$$self.$$.update = () => {
    		...
    		if ($$self.$$.dirty & /*interval*/ 128) {
    			 $$invalidate(5, speed = Math.floor(scale / interval * 100));
    		}
    	};

    	return [foodLeft, foodTop, direction, snakeBodies, score, speed, onKeyDown];
    }
